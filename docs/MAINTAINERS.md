...
2. **Build and push from `main`**:
   ...
   gem push trane-X.Y.Z.gem         # first time: prompts for email + password + MFA OTP code
   ```

   The first `gem push` signs you in and stores the credential locally
   (`~/.gem/credentials`). The very first push in the gem's history also
   claims the `trane` name on rubygems.org.
